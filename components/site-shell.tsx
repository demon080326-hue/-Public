...
function replyFor(value: string, language: SiteLanguage) {
  ...
  if (includes("claude")) return { text: response("我找到 Claude 相關文章與工具入口，你可以先從文章列表開始。", "I found Claude-related articles and tool entries. Start with the article list.", "Claude関連の記事とツール入口が見つかりました。まず記事一覧を確認してください。"), items: knowledge.filter((item) => item.href === "/articles" || item.href.includes("claude")) };
